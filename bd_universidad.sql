...
--
-- Host: 127.0.0.1:3306
-- Generation Time: May 28, 2022 at 01:43 AM
...
-- Procedures
--
DROP PROCEDURE IF EXISTS `nombre_notas`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `nombre_notas` ()  begin
...
DROP PROCEDURE IF EXISTS `NOMBRE_NOTA_POR_MATERIA`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `NOMBRE_NOTA_POR_MATERIA` ()  BEGIN
...
DROP PROCEDURE IF EXISTS `PROMEDIO_MATERIA`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `PROMEDIO_MATERIA` (IN `VAR_MATERIA` VARCHAR(100))  begin 
...
DROP PROCEDURE IF EXISTS `promedio_notas_estudiante`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `promedio_notas_estudiante` (IN `x_codigo_estudiante` VARCHAR(100), IN `x_codigo_materia` VARCHAR(100))  begin
...
-- Functions
--
DROP FUNCTION IF EXISTS `ASINATURAS_DE_PROFESOR`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `ASINATURAS_DE_PROFESOR` (`x_PROFESOR_CODIGO` VARCHAR(10)) RETURNS VARCHAR(100) CHARSET latin1 begin
...
DROP FUNCTION IF EXISTS `F_Asignatura`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `F_Asignatura` (`x_Codigo` VARCHAR(15)) RETURNS VARCHAR(100) CHARSET latin1 begin
...
DROP FUNCTION IF EXISTS `materias_dictadas_por_docente`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `materias_dictadas_por_docente` (`x_codigo_docente` VARCHAR(20)) RETURNS VARCHAR(1000) CHARSET latin1 begin
...
--
-- Table structure for table `grupos_por_materia`
--

DROP TABLE IF EXISTS `grupos_por_materia`;
...
--
-- Dumping data for table `grupos_por_materia`
--
...
--
-- Triggers `grupos_por_materia`
--
DROP TRIGGER IF EXISTS `T_ESTUDIANTEPERIODO`;
DELIMITER $$
CREATE TRIGGER `T_ESTUDIANTEPERIODO` BEFORE UPDATE ON `grupos_por_materia` FOR EACH ROW begin
...
-- Triggers `notas`
--
DROP TRIGGER IF EXISTS `nivel_desempeño_notas`;
DELIMITER $$
CREATE TRIGGER `nivel_desempeño_notas` BEFORE UPDATE ON `notas` FOR EACH ROW begin
...
--
-- Table structure for table `tipos_de_nota`
--

DROP TABLE IF EXISTS `tipos_de_nota`;
...
--
-- Dumping data for table `tipos_de_nota`
--
...
--
-- Constraints for table `grupos_por_materia`
--
ALTER TABLE `grupos_por_materia`
  ADD CONSTRAINT `FK_Estudiantes` FOREIGN KEY (`codigoEstudiante`) REFERENCES `estudiantes` (`codigoEstudiante`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `FK_Materias` FOREIGN KEY (`codigoMateria`) REFERENCES `materias` (`codigoMateria`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `FK_Periodos` FOREIGN KEY (`codigoPeriodo`) REFERENCES `periodos` (`codigoPeriodo`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `FK_Profesores` FOREIGN KEY (`codigoProfesor`) REFERENCES `profesores` (`codigoProfesor`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Constraints for table `notas`
--
ALTER TABLE `notas`
  ADD CONSTRAINT `FK_EstudiantesNotas` FOREIGN KEY (`codigoEstudiante`) REFERENCES `estudiantes` (`codigoEstudiante`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `FK_MateriasNotas` FOREIGN KEY (`codigoMateria`) REFERENCES `materias` (`codigoMateria`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `FK_PeriodosNotas` FOREIGN KEY (`codigoPeriodo`) REFERENCES `periodos` (`codigoPeriodo`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `FK_TiposNotas` FOREIGN KEY (`codigoTipoNota`) REFERENCES `tipos_de_nota` (`codigoTipoNota`) ON DELETE NO ACTION ON UPDATE NO ACTION;
COMMIT;
